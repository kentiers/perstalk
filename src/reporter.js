...
export async function generateChangelog() {
  ...
  md += `| Akun | Nama | Bio | Video | Followers | Following | Likes | Story | Tanggal Dicek |\n`;
  md += `|---|---|---|---|---|---|---|---|---|\n`;
...
<head>
  ...
  <style>
    :root {
      ...
      --panel-inner: #191b24;
      --panel-hover: #20222f;
      --panel-selected: #252837;
      --border: #282a3a;
      --border-subtle: #1e202d;
      --text: #ffffff;
      --text-muted: #cbd5e1;
      --text-dim: #9499ab;
      --accent: #38bdf8;
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .tag-badge.change { background: rgba(245, 158, 11, 0.2); color: var(--warning); }
    .tag-badge.story { background: var(--accent-soft); color: var(--accent); }
    .tag-badge.base { background: rgba(255, 255, 255, 0.1); color: #e2e8f0; }
    ...
    .diff-new { color: var(--positive); font-weight: 700; }
    .diff-delta { color: var(--accent); font-size: 13px; font-weight: 700; margin-left: 6px; }
...
<body>
  ...
  <header class="topbar">
    ...
    <div class="topbar-stats">
      <span>Total Video: <strong id="sum-videos">0</strong></span>
      <span>Followers: <strong id="sum-followers">0</strong></span>
      <span>Story Aktif: <strong id="sum-stories" style="color:var(--accent)">0</strong></span>
    </div>
  ...
  <script>
    ...
    function renderStage() {
      ...
      if (!selectedUser || !accounts[selectedUser]) {
        stage.innerHTML = '<div style="padding:40px;text-align:center;color:var(--text-dim);font-size:16px;">Pilih akun di panel kiri.</div>';
        return;
      }
      ...
      if (userEvents.length === 0) {
        ...
        rowsHtml = rev.map(item => {
          ...
          if (item.eventType === 'INITIAL_BASELINE') {
            ...
            detail = \`Story: "\${escapeHtml(item.story.desc || 'Tanpa teks')}" <a href="\${item.story.url}" target="_blank" style="color:var(--accent);text-decoration:none;font-weight:600;margin-left:6px;">[Buka Link]</a>\`;
        ...
        <div class="stage-content">
          ...
          <div class="target-history-box">
            <div class="target-history-header">
              ...
              <span style="font-size:13.5px;color:var(--text-muted);font-weight:600;">\${userEvents.length} Peristiwa</span>
            </div>
